...
import type { ExtLoan, ExtLoanTxn } from '@/types/db';

export default function ExternalLoans() {
  const [loans, setLoans] = useState<ExtLoan[]>([]);
  const [txns, setTxns] = useState<ExtLoanTxn[]>([]);
  ...
  const [paymentType, setPaymentType] = useState<'Interest Paid' | 'Principal Paid'>('Interest Paid');
  ...
  const fetchData = async () => {
    ...
    try {
      // list_ext_loans auto-generates monthly interest dues server-side, so
      // we always get a fresh ledger.
      const [l, t] = await Promise.all([api.listExtLoans(), api.listExtLoanTxns()]);
      setLoans(l); setTxns(t);
    } catch (err) {
  ...
  const reset = () => {
    ...
    setBorrowerName(''); setPhone(''); setAddress(''); setIdProof('');
    setPrincipal(''); setInterestRate('');
    setStartDate(format(new Date(), 'yyyy-MM-dd'));
    setStatus('Active');
  };

  const openEdit = (l: ExtLoan) => {
    setIsEdit(true); setSelectedId(l.id);
    setBorrowerName(l.borrower_name);
    setPhone(l.phone || ''); setAddress(l.address || '');
    setIdProof(l.id_proof || '');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await api.updateExtLoan(selectedId, {
          borrower_name: borrowerName.trim(),
          phone: phone.trim() || null,
          address: address.trim() || null,
          id_proof: idProof.trim() || null,
          interest_rate: Number(interestRate),
          status,
        });
        setSuccess('Borrower updated.');
      } else {
        await api.createExtLoan({
          borrower_name: borrowerName.trim(),
          phone: phone.trim() || null,
          address: address.trim() || null,
          id_proof: idProof.trim() || null,
        ...
        setSuccess('Borrower & loan added.');
      }
  ...
  const handleDelete = async () => {
    setFormLoading(true); setError('');
    try {
      await api.deleteExtLoan(selectedId);
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    setFormLoading(true); setError('');
    try {
      const txn = await api.addExtLoanPayment({
        loan_id: selectedId,
        type: paymentType,
      ...
      setSuccess(`${paymentType} recorded · Receipt ${txn.receipt_number}`);
  ...
  const selectedLoanTxns = txns.filter((t) => t.loan_id === selectedId);
  const totalInterestDue = selectedLoanTxns.filter((t) => t.type === 'Interest Due').reduce((s, t) => s + t.amount, 0);
  const totalInterestPaid = selectedLoanTxns.filter((t) => t.type === 'Interest Paid').reduce((s, t) => s + t.amount, 0);
  const totalPrincipalPaid = selectedLoanTxns.filter((t) => t.type === 'Principal Paid').reduce((s, t) => s + t.amount, 0);
  const currentPrincipal = selectedLoan ? selectedLoan.principal_amount - totalPrincipalPaid : 0;
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr><td colSpan={6} className="p-8 text-center text-gray-500">No external personal loans.</td></tr>
              ) : loans.map((loan) => (
                <tr key={loan.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4 text-gray-600">
                    ...
                    <div className="text-xs truncate max-w-[150px]" title={loan.address || ''}>
                      <i className="fas fa-map-marker-alt mr-1 text-gray-400"></i> {loan.address || '-'}
                    </div>
                  </td>
                  ...
                  <td className="p-4 text-right space-x-2 whitespace-nowrap">
                    <button
                      onClick={() => { setSelectedId(loan.id); setIsLedgerOpen(true); }}
                      className="text-sm text-[#1e5a48] hover:text-[#0b3b2f] font-medium bg-[#1e5a48]/10 hover:bg-[#1e5a48]/20 px-3 py-1.5 rounded-lg"
                    >Ledger &amp; Payments</button>
                    <button onClick={() => openEdit(loan)} className="text-sm text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg" title="Edit">
                      <i className="fas fa-edit"></i>
                    </button>
                    <button onClick={() => { setSelectedId(loan.id); setIsDeleteOpen(true); }} className="text-sm text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg" title="Delete">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b flex justify-between items-center bg-[#0b3b2f] text-white">
              <h3 className="font-bold text-lg">{isEdit ? 'Edit Borrower &amp; Loan' : 'New External Borrower'}</h3>
              <button onClick={() => setIsAddOpen(false)} className="text-white/70 hover:text-white">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={idProof} onChange={(e) => setIdProof(e.target.value)} placeholder="Aadhar / PAN" />
                  </div>
                  <div className="space-y-2 md:col-span-2">
                    <Label>Address</Label>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" value={principal} onChange={(e) => setPrincipal(e.target.value)} required min="1" disabled={isEdit} />
                    {isEdit && <p className="text-xs text-gray-500">Principal cannot be changed after creation. Use principal payments in ledger.</p>}
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2 md:col-span-2">
                    ...
                    <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} required disabled={isEdit} />
                  </div>
                  {isEdit && (
                    <div className="space-y-2 md:col-span-2">
                ...
                <div className="pt-4 flex justify-end gap-3 border-t mt-6">
                  ...
                  <Button type="submit" disabled={formLoading}>{formLoading ? 'Saving…' : 'Save Details'}</Button>
                </div>
              </form>
            </div>
          </div>
      ...
      {/* Ledger Modal */}
      {isLedgerOpen && selectedLoan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl overflow-hidden flex flex-col h-[90vh]">
            <div className="p-5 border-b flex justify-between items-center bg-[#1e5a48] text-white">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setIsLedgerOpen(false)} className="text-white/70 hover:text-white">
                  <i className="fas fa-times text-xl"></i>
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-80 bg-gray-50 p-6 flex flex-col border-t md:border-t-0 print:hidden">
                ...
                <form onSubmit={handleAddPayment} className="space-y-4 flex-1">
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      value={paymentType}
                      onChange={(e) => setPaymentType(e.target.value as 'Interest Paid' | 'Principal Paid')}
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      required min="1"
                      max={paymentType === 'Interest Paid' ? balanceInterest + 5000 : currentPrincipal}
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={paymentNotes} onChange={(e) => setPaymentNotes(e.target.value)} placeholder="Cash, Bank Transfer…" />
                  </div>

                  <div className="pt-4">
                    <Button type="submit" className="w-full bg-[#1e5a48] hover:bg-[#154636] text-white" disabled={formLoading}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-6">This will delete all transactions for this loan. Cannot be undone.</p>
